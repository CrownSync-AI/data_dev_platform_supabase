...
type PlatformStatus = 'posted' | 'scheduled' | 'missing' | 'pending'
...
const retailerPostingData: RetailerData[] = [
    {
        ...
        platforms: {
            ...
            x: {
                status: 'scheduled',
                scheduledTime: '2025-03-20 10:00',
                reason: 'Scheduled for product launch day'
            },
            linkedin: {
                status: 'posted',
    ...
    },
    {
        ...
        platforms: {
            ...
            linkedin: {
                status: 'scheduled',
                scheduledTime: '2025-03-18 14:00',
                reason: 'Aligned with B2B outreach campaign'
            }
        }
    },
    {
        ...
        platforms: {
            ...
            facebook: {
                status: 'pending',
                reason: 'Awaiting final creative approval from brand team'
            },
            x: {
                status: 'missing',
                reason: 'Content not submitted by retailer'
            },
            linkedin: {
                status: 'posted',
    ...
    },
    {
        ...
        platforms: {
            instagram: {
                status: 'scheduled',
                scheduledTime: '2025-03-19 16:00',
                reason: 'Scheduled for weekend engagement peak'
            },
            facebook: {
                status: 'posted',
    ...
    },
    {
        ...
        platforms: {
            ...
            facebook: {
                status: 'scheduled',
                scheduledTime: '2025-03-21 11:00',
                reason: 'Coordinated with email campaign launch'
            },
            x: {
                status: 'posted',
...
function StatusCell({ retailer, platformId, onCellClick }: StatusCellProps) {
    ...
    const status = platformData?.status || 'missing'
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                    {status === 'posted' && platformData ? (
                    ...
                    ) : status === 'scheduled' && platformData ? (
                        <>
                            <div className="p-4 bg-orange-50 rounded-lg border border-orange-100">
                                <div className="flex items-center gap-2 mb-2">
                                    <Clock className="w-4 h-4 text-orange-600" />
                                    <span className="font-medium text-orange-900">Scheduled For</span>
                                </div>
                                <p className="text-sm font-semibold text-orange-700">{platformData.scheduledTime}</p>
                            </div>
                            {platformData.reason && (
                                <div>
                                    <p className="text-xs text-gray-500 mb-1">Note</p>
                                    <p className="text-sm text-gray-700">{platformData.reason}</p>
                                </div>
                            )}
                        </>
                    ) : status === 'pending' && platformData ? (
                        ...
                        <div className="p-4 bg-red-50 rounded-lg border border-red-100">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="font-medium text-red-900">Content Missing</span>
                            </div>
                            {platformData?.reason && (
...
export default function PlatformPostingMatrix() {
    ...
    retailerPostingData.forEach(retailer => {
        platforms.forEach(platform => {
            const status = retailer.platforms[platform.id]?.status || 'missing'
            statusCounts[status as keyof typeof statusCounts]++
        })
    })

    const completionRate = ((statusCounts.posted / totalCells) * 100).toFixed(1)
        ...
        <>
            <Card className="overflow-hidden border border-gray-200 shadow-sm">
                ...
                <CardContent className="p-0">
                    ...
                    {/* Summary Statistics */}
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 p-6 bg-white border-t border-gray-200">
                        <div className="text-center">
                        ...
                        <div className="text-center">
                            <div className="flex items-center justify-center gap-2 mb-1">
                                <div className={`w-2 h-2 rounded-full ${statusConfig.scheduled.dotColor}`} />
                                <p className="text-xs font-medium text-gray-600 uppercase tracking-wide">Scheduled</p>
                            </div>
                            <p className="text-2xl font-bold text-orange-600">{statusCounts.scheduled}</p>
                        </div>
                        <div className="text-center">
                            <div className="flex items-center justify-center gap-2 mb-1">
                                <div className={`w-2 h-2 rounded-full ${statusConfig.pending.dotColor}`} />
                        ...
                        <div className="text-center">
                            <div className="flex items-center justify-center gap-2 mb-1">
                                <div className={`w-2 h-2 rounded-full ${statusConfig.missing.dotColor}`} />
                                <p className="text-xs font-medium text-gray-600 uppercase tracking-wide">Missing</p>
                            </div>
