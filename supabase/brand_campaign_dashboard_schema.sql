-- Brand Campaign Dashboard Schema
...
CREATE TABLE IF NOT EXISTS brand_campaign_cards (
...
CREATE TABLE IF NOT EXISTS campaign_detailed_metrics (
...
CREATE TABLE IF NOT EXISTS campaign_platform_performance (
...
CREATE TABLE IF NOT EXISTS campaign_retailer_summary (
...
CREATE TABLE IF NOT EXISTS campaign_insights (
    ...
    optimal_posting_times TEXT[], -- Array of time strings
...
CREATE INDEX IF NOT EXISTS idx_brand_campaign_cards_brand_id ON brand_campaign_cards(brand_id);
CREATE INDEX IF NOT EXISTS idx_brand_campaign_cards_status ON brand_campaign_cards(campaign_status);
CREATE INDEX IF NOT EXISTS idx_brand_campaign_cards_type ON brand_campaign_cards(campaign_type);
CREATE INDEX IF NOT EXISTS idx_brand_campaign_cards_dates ON brand_campaign_cards(start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_brand_campaign_cards_performance ON brand_campaign_cards(performance_tier, roi_percentage);

-- Platform Performance Indexes
CREATE INDEX IF NOT EXISTS idx_campaign_platform_performance_campaign ON campaign_platform_performance(campaign_id);
CREATE INDEX IF NOT EXISTS idx_campaign_platform_performance_platform ON campaign_platform_performance(platform);
CREATE INDEX IF NOT EXISTS idx_campaign_platform_performance_date ON campaign_platform_performance(analytics_date);

-- Daily Metrics Indexes
CREATE INDEX IF NOT EXISTS idx_campaign_daily_metrics_campaign ON campaign_daily_metrics(campaign_id);
...
CREATE POLICY "Brands can view their campaign insights" ON campaign_insights
    FOR SELECT USING (
        campaign_id IN (
            SELECT campaign_id FROM brand_campaign_cards 
            WHERE brand_id = auth.uid()
...
CREATE OR REPLACE VIEW brand_campaign_dashboard AS
SELECT 
    bcc.*,
    cdm.emails_sent,
    cdm.delivery_rate,
...
-- =====================================================
-- 10. TRIGGER FUNCTIONS FOR CALCULATED FIELDS
-- =====================================================
