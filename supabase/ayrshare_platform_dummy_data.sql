-- Ayrshare Platform-Specific Dummy Data
-- Comprehensive dummy data based on actual Ayrshare API structure
-- This populates the enhanced platform-specific metrics tables

-- First, ensure we have the base data from the original schema
-- This script assumes the base campaign_performance_new_dummy_data.sql has been run
...
-- Insert Instagram metrics (based on Ayrshare Instagram structure)
INSERT INTO platform_specific_metrics (
    post_id, campaign_id, account_id, platform, analytics_date,
    impressions, reach, likes, comments, shares,
...
SELECT 
    ...
    -- Core metrics (Instagram typically has good engagement)
...
-- Insert Twitter/X metrics (based on Ayrshare Twitter structure)
INSERT INTO platform_specific_metrics (
    post_id, campaign_id, account_id, platform, analytics_date,
    impressions, reach, likes, comments, shares,
...
SELECT 
    ...
    -- Core metrics (Twitter has different engagement patterns)
    ...
    jsonb_build_object(
        ...
        'analytics', jsonb_build_object(
            ...
            'nonPublicMetrics', jsonb_build_object(
                'userProfileClicks', (random() * 20 + 3)::integer,
                'engagements', (random() * 100 + 20)::integer,
                'video', jsonb_build_object(
                    'playback25Count', (random() * 250 + 40)::integer,
                    'playback50Count', (random() * 180 + 30)::integer,
                    'playback75Count', (random() * 120 + 20)::integer,
                    'playback100Count', (random() * 80 + 15)::integer
                )
            )
        )
    ) as ayrshare_raw_data
...
-- Insert LinkedIn metrics (based on Ayrshare LinkedIn structure)
INSERT INTO platform_specific_metrics (
    post_id, campaign_id, account_id, platform, analytics_date,
    impressions, reach, likes, comments, shares,
...
SELECT 
    ...
    -- Core metrics (LinkedIn has professional engagement)
...
-- Generate platform performance summaries for each campaign and platform
...
-- =====================================================
-- 4. ADD SAMPLE POST CONTENT AND METADATA
-- =====================================================

-- Update campaign posts with realistic content
UPDATE campaign_posts_new 
SET 
    post_content = CASE 
        WHEN random() > 0.7 THEN 'Discover luxury redefined. Our latest collection embodies elegance and sophistication. #LuxuryLifestyle #Elegance'
        WHEN random() > 0.4 THEN 'Crafted with precision, designed for perfection. Experience the art of luxury. #Craftsmanship #LuxuryDesign'
        ELSE 'Where tradition meets innovation. Explore our exclusive collection. #Heritage #Innovation #Luxury'
    END,
    post_type = CASE 
        WHEN random() > 0.8 THEN 'video'
        WHEN random() > 0.6 THEN 'carousel'
        WHEN random() > 0.3 THEN 'image'
        ELSE 'story'
    END,
    hashtags = ARRAY['#luxury', '#elegance', '#premium', '#exclusive'],
    post_metadata = jsonb_build_object(
        'campaign_theme', 'luxury_lifestyle',
        'target_audience', 'affluent_consumers',
        'content_pillar', CASE 
            WHEN random() > 0.6 THEN 'product_showcase'
            WHEN random() > 0.3 THEN 'brand_story'
            ELSE 'lifestyle_inspiration'
        END
    )
WHERE post_content IS NULL;
...
DO $$
DECLARE
    platform_metrics_count INTEGER;
    platform_summary_count INTEGER;
    email_campaigns_count INTEGER;
    rec RECORD;
BEGIN
    SELECT COUNT(*) INTO platform_metrics_count FROM platform_specific_metrics;
    SELECT COUNT(*) INTO platform_summary_count FROM platform_performance_summary;
    SELECT COUNT(*) INTO email_campaigns_count FROM email_campaigns_new;
    
    RAISE NOTICE 'Data insertion completed:';
    RAISE NOTICE '- Platform Specific Metrics: % records', platform_metrics_count;
    RAISE NOTICE '- Platform Performance Summary: % records', platform_summary_count;
    RAISE NOTICE '- Email Campaigns: % records', email_campaigns_count;
    ...
    FOR rec IN 
        ...
        ORDER BY platform
    LOOP
        RAISE NOTICE '- % platform: % metrics', rec.platform, rec.count;
    END LOOP;
END $$;

-- Sample queries to verify data quality
SELECT 
    'Platform Performance Overview' as report_type,
    platform,
...
SELECT 
    'Top Performing Campaigns by Platform' as report_type,
    c.campaign_name,
    psm.platform,
    ROUND(AVG(psm.engagement_rate), 2) as avg_engagement_rate,
    SUM(psm.total_engagement) as total_engagement
FROM platform_specific_metrics psm
JOIN campaigns_new c ON psm.campaign_id = c.campaign_id
GROUP BY c.campaign_name, psm.platform
ORDER BY avg_engagement_rate DESC
LIMIT 10;